...
function t4() {
    let a4_arr = [];
    a4_res = a4.map(function(elem, index, arr) {
        if (typeof elem === 'number'){
            a4_arr.push(elem);
        }
    });
    a4_res = a4_arr;
    return a4_res; //!!! не понимаю почему выводит элементы underfined
}
...
function t8() {
    let b8_arr = [];
    b8_res = b8.filter((elem, index, arr) => {
...
function t10() {
    b10_res = b10.filter(elem => {
        if (elem.includes(3)) return elem;
    })
    return b10_res;
}
