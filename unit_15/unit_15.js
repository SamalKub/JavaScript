...
const f12 = () => {
    console.log(new Set(str12)); // сначала я решила сделать это задание в одну строчку, но потом подумала, что может быть нужно сделать четко по инструкции и сравнить полученные результаты.

    let len = str12.length;
    let out = [];
    for (let i = 0; i < len; i++){
        out.push(str12[i]);
    }
    let s12 = new Set(out);
    return(s12); // итог: никакой разницы
}
